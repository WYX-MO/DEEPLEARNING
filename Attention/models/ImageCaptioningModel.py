...
class ImageCaptioningModel(nn.Module):
    def __init__(self, vocab_size,max_seq_len):
        ...
        self.decoder_block = DecoderBlock(192, 192, 3)  # Initialize the decoder block
    ...
    def forward(self, images, captions ):
        ...
        decoder_output = self.decoder_block(image_features, text_features, mask[:text_features.shape[1], :text_features.shape[1]])  # Pass image and text features through the decoder block
...
if __name__ == "__main__":
    vocab_size = 10000
    model = ImageCaptioningModel(vocab_size, 32)

    images = torch.randn(2, 3, 32, 32)
    captions = torch.randint(0, vocab_size, (2, 10))
